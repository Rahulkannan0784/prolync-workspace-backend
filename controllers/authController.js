...
export const getMe = async (req, res) => {
    try {
        ...
        if (role === 'ADMIN') {
            ...
            const [users] = await db.query('SELECT id, custom_id, name, email, phone_number, college_name, department, profile_picture, resume_path, role, created_at, bio, location, github, linkedin, gender, current_role, status, valid_until FROM users WHERE id = ?', [req.user.id]);
